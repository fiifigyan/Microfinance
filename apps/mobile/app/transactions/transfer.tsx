...
export default function TransferScreen() {
  ...
  const handleTransfer = async () => {
    ...
    if (!from) {
      showAlert("Error", "Please select a source account", "warning");
      return;
    }

    if (!to) {
      showAlert("Error", "Please select a destination account", "warning");
      return;
    }

    const numAmount = parseFloat(amount);
    if (isNaN(numAmount) || numAmount < 1) {
      showAlert("Error", "Please enter a valid amount (minimum GHS 1)", "warning");
      return;
    }
    ...
    try {
      ...
      showAlert("Error", error.message || "Failed to transfer funds", "error");
    } finally {
      setLoading(false);
    }
  };
