...
export default function DepositScreen() {
  ...
  const handleDeposit = async () => {
    ...
    if (!validAmount) {
      showAlert("Invalid Amount", "Minimum deposit is GHS 1.", "warning");
      return;
    }
    ...
    try {
      ...
      showAlert(
        "Deposit Initiated",
        `GHS ${parsedAmount.toFixed(2)} deposit is being processed.\n\nReference: ${result.reference}`,
        "success",
        () => router.back(),
      );
    } catch (err: any) {
      showAlert("Deposit Failed", err.message || "Something went wrong. Please try again.", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <View style={styles.header}>
        <View style={styles.headerIcon}>
          <Ionicons name="arrow-down" size={28} color="#10B981" />
        </View>
        <Text style={styles.title}>Deposit Funds</Text>
        <Text style={styles.subtitle}>Add money via Mobile Money</Text>
      </View>
      ...
      <View style={styles.btnContainer}>
        <TouchableOpacity
          style={[
            styles.btn,
            (!validAmount || !phoneNumber || loading) && styles.btnDisabled,
          ]}
        ...
        >
          {loading ? (
            ...
            <Text style={styles.btnText}>Deposit Now</Text>
          )}
        </TouchableOpacity>
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  dropdownItemActive: { borderColor: "#2563EB", backgroundColor: "#EFF6FF" },
  dropdownItemText: { fontSize: 14, color: "#1F2937" },
  dropdownBalance: { fontSize: 14, fontWeight: "600", color: "#1F2937" },
  amountRow: {
    backgroundColor: "#FFFFFF",
    borderWidth: 1,
    borderColor: "#E5E7EB",
  ...
  quickBtnActive: { backgroundColor: "#2563EB" },
  ...
  summaryTotalLabel: { fontSize: 16, fontWeight: "700", color: "#166534" },
